...
static void lps22hb_thread(struct lps22hb_data *lps22hb)
{	
	while (1) {
...
{
	struct lps22hb_data *lps22hb = 
		CONTAINER_OF(work, struct lps22hb_data, work);
...
int lps22hb_init_interrupt(const struct device *dev)
{
	...
	k_thread_create(&lps22hb->thread, lps22hb->thread_stack,
		       CONFIG_LPS22HB_THREAD_STACK_SIZE,
		       (k_thread_entry_t)lps22hb_thread, lps22hb,
		       NULL, NULL, K_PRIO_COOP(CONFIG_LPS22HB_THREAD_PRIORITY),
		       0, K_NO_WAIT);
#elif defined(CONFIG_LPS22HB_TRIGGER_GLOBAL_THREAD)
	...
	gpio_init_callback(&lps22hb->gpio_cb, 
				lps22hb_gpio_callback,
	...
	if (lps22hb_int_notification_mode_set(ctx, LPS22HB_INT_PULSED) < 0) {
